...
export default function InventoryManager({ equipment, categories = [] }) {
  ...
  const groups = useMemo(() => {
    ...
    // 각 그룹 내 번호순 정렬 (가챠2가 가챠10 앞에 오도록)
    const num = (s) => {
      const m = String(s).match(/(\d+)\s*$/);
      return m ? parseInt(m[1], 10) : 0;
    };
    for (const [, items] of map) {
      items.sort((a, b) => num(a.name) - num(b.name) || a.name.localeCompare(b.name));
    }
    return [...map.entries()];
  }, [equipment]);
  ...
  const onAdd = (e) => {
    ...
    run(async () => {
      const res = await createEquipmentBulk({ category, count });
      if (!res?.error) setCount("1");
      return res;
    });
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={onAdd} className="rounded-xl border border-ink/10 bg-white p-4">
        <p className="mb-1 text-sm font-bold text-ink">기기 등록</p>
        <p className="mb-2.5 text-xs text-ink/45">
          종류와 수량만 넣으면 번호를 붙여 자동 생성합니다. (예: 가챠머신 · 7 →
          가챠머신1~가챠머신7)
        </p>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-1.5">
            <input
              ...
              className="w-20 rounded-md border border-ink/15 px-3 py-2.5 text-sm outline-none focus:border-primary"
            />
                ...
                <div className="flex items-center gap-2">
                  ...
                  {cat !== "미분류" && (
                    <button
                      type="button"
                      disabled={pending}
                      onClick={() => run(() => addNextUnit({ category: cat }))}
                      className="rounded-md border border-ink/15 px-2 py-1 text-xs font-medium text-ink/70 hover:bg-ink/5 disabled:opacity-50"
                    >
                      +1대
                    </button>
                  )}
